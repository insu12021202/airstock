...
public class ListViewAdapter extends BaseAdapter {
    ...
    ArrayList<ListItemAdapterData> list = new ArrayList<ListItemAdapterData>();
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        TextView itemNum = (TextView)view.findViewById(R.id.item_num);
        TextView itemDate = (TextView)view.findViewById(R.id.item_date);

        //현재 포지션에 해당하는 아이템에 글자를 적용하기 위해 list배열에서 객체를 가져온다.
        ListItemAdapterData listdata = list.get(i);

        //가져온 객체안에 있는 글자들을 각 뷰에 적용한다
        itemNum.setText(listdata.getNum());
        itemName.setText(listdata.getName());
        itemDate.setText(listdata.getDate());

        return view;
    }

    //ArrayList로 선언된 list 변수에 목록을 채워주기 위함 다른방시으로 구현해도 됨
    public void addItemToList( String name, String num, String date){
        ListItemAdapterData listdata = new ListItemAdapterData();
